...
export async function updateWeightLog(id: string, weightKg: number) {
  ...
  if (!session) {
    throw new Error("You must be signed in to update weight logs.");
  }
...
export async function deleteWeightLog(id: string) {
  ...
  if (!session) {
    throw new Error("You must be signed in to delete weight logs.");
  }
...
export async function updateAppConfig(formData: FormData) {
  ...
  const service = createSupabaseServiceClient();
  if (!service) {
    throw new Error("Set SUPABASE_SERVICE_ROLE_KEY to update app config.");
  }
